...
function localizeFooter(pack) {
  ...
  if (rights) {
    ...
    rights.innerHTML = `© <span data-current-year>${year}</span> Hakkı Burak Bakırcı. ${pack.footerRights || 'All rights reserved.'}`;
  }
...
const PRX_CONTACT_LABELS = {
  en: ['Full name','Email address','Message','Send message','Official contact email','Created by','Rights notice'],
  tr: ['Ad soyad','E-posta adresi','Mesaj','Mesaj gönder','Resmi iletişim e-postası','Oluşturan','Hak bildirimi'],
  es: ['Nombre completo','Correo electrónico','Mensaje','Enviar mensaje','Correo oficial de contacto','Creado por','Aviso de derechos'],
  'pt-BR': ['Nome completo','Endereço de e-mail','Mensagem','Enviar mensagem','E-mail oficial de contato','Criado por','Aviso de direitos'],
  de: ['Vollständiger Name','E-Mail-Adresse','Nachricht','Nachricht senden','Offizielle Kontakt-E-Mail','Erstellt von','Rechtshinweis'],
  fr: ['Nom complet','Adresse e-mail','Message','Envoyer le message','E-mail de contact officiel','Créé par','Avis de droits'],
  it: ['Nome completo','Indirizzo e-mail','Messaggio','Invia messaggio','Email ufficiale di contatto','Creato da','Avviso sui diritti'],
  id: ['Nama lengkap','Alamat email','Pesan','Kirim pesan','Email kontak resmi','Dibuat oleh','Pemberitahuan hak'],
  ja: ['氏名','メールアドレス','メッセージ','メッセージを送信','公式連絡先メール','作成者','権利表示'],
  ko: ['이름','이메일 주소','메시지','메시지 보내기','공식 연락 이메일','제작자','권리 고지'],
  'zh-CN': ['姓名','电子邮件地址','消息','发送消息','官方联系邮箱','创建者','权利声明'],
  ar: ['الاسم الكامل','البريد الإلكتروني','الرسالة','إرسال الرسالة','البريد الرسمي للتواصل','تم الإنشاء بواسطة','إشعار الحقوق'],
  hi: ['पूरा नाम','ईमेल पता','संदेश','संदेश भेजें','आधिकारिक संपर्क ईमेल','निर्माता','अधिकार सूचना'],
  ru: ['Полное имя','Адрес электронной почты','Сообщение','Отправить сообщение','Официальный контактный e-mail','Создано','Уведомление о правах']
};
...
function localizeLegalPages(c) {
  ...
  if (page === 'privacy') {
    ...
    const labels = legal.querySelectorAll('label');
    setTexts(labels, [l.fullName,l.email,l.message]);
    setTexts(legal.querySelectorAll('button'), l.send);
    setTexts(legal.querySelectorAll('h2'), [l.official,l.created,l.rights]);
  }
}
...
function navIconSvg(kind) {
  const icons = {
    home: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M4.75 10.2 12 4.5l7.25 5.7v8.15a1.1 1.1 0 0 1-1.1 1.1h-3.7v-5.35H9.55v5.35h-3.7a1.1 1.1 0 0 1-1.1-1.1z"/><path class="nav-accent" d="M9.55 19.45v-5.35h4.9v5.35"/></svg>`,
    convert: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M7.5 6.5h10.2"/><path class="nav-path" d="m14.65 3.6 3.05 2.9-3.05 3.1"/><path class="nav-path" d="M16.5 17.5H6.3"/><path class="nav-path" d="m9.35 14.4-3.05 3.1 3.05 2.9"/><circle class="nav-dot" cx="12" cy="12" r="1.25"/></svg>`,
    compress: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M7.75 5.5v2.6M7.75 15.9v2.6M4.5 9.25h2.6M10.35 9.25h2.6"/><path class="nav-path" d="M16.25 5.5v13"/><path class="nav-path" d="m13.6 15.9 2.65 2.6 2.65-2.6"/></svg>`,
    resize: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M8.1 4.75H4.75V8.1"/><path class="nav-path" d="M15.9 4.75h3.35V8.1"/><path class="nav-path" d="M8.1 19.25H4.75V15.9"/><path class="nav-path" d="M15.9 19.25h3.35V15.9"/><path class="nav-path" d="M9 7H7v2M15 7h2v2M9 17H7v-2M15 17h2v-2"/></svg>`,
    crop: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M7 4.75v11.5a1.75 1.75 0 0 0 1.75 1.75h11.5"/><path class="nav-path" d="M4.75 7h11.5A1.75 1.75 0 0 1 18 8.75v10.5"/><path class="nav-accent" d="M9.6 9.6h4.8v4.8H9.6z"/></svg>`,
    pdf: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M8 4.75h6.85l3.4 3.4v10.1A1.75 1.75 0 0 1 16.5 20H8a1.75 1.75 0 0 1-1.75-1.75V6.5A1.75 1.75 0 0 1 8 4.75Z"/><path class="nav-path" d="M14.85 4.75v3.4h3.4"/><path class="nav-accent" d="M8.85 15.8h6.3"/><path class="nav-accent" d="M8.85 12.8h4.9"/></svg>`,
    split: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M5.25 5.25h13.5v13.5H5.25z"/><path class="nav-accent" d="M12 5.25v13.5M5.25 12h13.5"/><path class="nav-path" d="m8 8 1.2 1.2M16 8l-1.2 1.2M8 16l1.2-1.2M16 16l-1.2-1.2"/></svg>`,
    formats: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M7 7.25h10"/><path class="nav-path" d="M7 12h10"/><path class="nav-path" d="M7 16.75h10"/><circle class="nav-dot" cx="5.2" cy="7.25" r="1"/><circle class="nav-dot" cx="18.8" cy="12" r="1"/><circle class="nav-dot" cx="5.2" cy="16.75" r="1"/></svg>`,
    guides: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="m12 4.75 1.85 4.25 4.4.4-3.3 2.95.95 4.35L12 14.45 8.1 16.7l.95-4.35-3.3-2.95 4.4-.4z"/><circle class="nav-dot" cx="12" cy="12" r="1.1"/></svg>`,
    faq: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M12 18.9a6.9 6.9 0 1 1 0-13.8 6.9 6.9 0 0 1 0 13.8Z"/><path class="nav-path" d="M9.85 10.1a2.35 2.35 0 1 1 4.05 1.65c-.8.75-1.45 1.25-1.45 2.15"/><circle class="nav-dot" cx="12" cy="15.95" r="1.1"/></svg>`,
    contact: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><path class="nav-path" d="M5.5 7h13a1.25 1.25 0 0 1 1.25 1.25v7.5A1.25 1.25 0 0 1 18.5 17h-13a1.25 1.25 0 0 1-1.25-1.25v-7.5A1.25 1.25 0 0 1 5.5 7Z"/><path class="nav-path" d="m5 8 7 5 7-5"/><circle class="nav-dot" cx="18.3" cy="7.7" r="1.15"/></svg>`,
    tool: `<svg class="nav-icon-svg" viewBox="0 0 24 24" aria-hidden="true"><circle class="nav-path" cx="12" cy="12" r="6.9"/><path class="nav-accent" d="M12 8.5v7M8.5 12h7"/></svg>`
  };
